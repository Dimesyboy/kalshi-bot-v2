...
def get_dynamic_config(client):
    """Scale position size and max open positions to live balance automatically."""
    try:
        ...
        log.debug(f"[Sizing] Balance=${balance:.2f} -> max_pos=${max_pos:.2f} max_open={max_open}")
        return max_pos, max_open
    except Exception as e:
        log.warning(f"[Sizing] Dynamic config failed: {e}")
        return Config.MAX_POSITION_USD, Config.MAX_OPEN_POSITIONS
...
SERIES_MAP = {}
# SERIES_MAP is intentionally restricted to ALLOWED_SERIES only.
# Never fetch series the strategies cannot trade — they become
# watcher targets and cause unguarded entries.
...
def _fetch_series(series_ticker: str, label: str) -> tuple:
    """
    Fetch via /markets endpoint directly — no status filter.
    The /events endpoint returns 400 for status=active on sports series.
    We filter by market status after fetching instead.
    """
    all_events         = []
    ...
    while page < 4:  # max 4 pages = 200 markets per series
        try:
            params = {
                "series_ticker": series_ticker,
                "limit":         50,
                "status":        "open",  # open = pre-game, active = live
            }
            if cursor:
            ...
            # Group markets back into pseudo-events by event_ticker
            for m in markets:
                m_status = m.get("status", "open")
                # Only trade open (pre-game) and active (live) markets
                if m_status not in ("open", "active"):
                    continue
                m["market_status"] = m_status

                # Derive event ticker from market ticker
                ticker = m.get("ticker", "")
                parts  = ticker.rsplit("-", 1)
                eticker = parts[0] if len(parts) == 2 else ticker

                if eticker not in seen_event_tickers:
                    seen_event_tickers.add(eticker)
                    all_events.append({
                        "event_ticker": eticker,
                        "title":        m.get("title", eticker),
                        "close_time":   m.get("close_time"),
                        "markets":      [],
                    })

                # Add market to its event
                for ev in all_events:
                    if ev["event_ticker"] == eticker:
                        ev["markets"].append(m)
                        break
            ...
            log.warning(f"[Fetcher] {series_ticker}: HTTP error {e}")
            break
        except Exception as e:
            log.warning(f"[Fetcher] {series_ticker}: {e}")
            break

    # Second pass — also fetch active markets
    # Small delay to avoid 429 rate limiting between the two passes
    time.sleep(Config.FETCH_DELAY_SECS)
    try:
        r = requests.get(
            f"{Config.KALSHI_BASE}/markets",
            params={"series_ticker": series_ticker, "limit": 50, "status": "active"},
            timeout=10,
        )
        if r.ok:
            for m in r.json().get("markets", []):
                m["market_status"] = "active"
                ticker  = m.get("ticker", "")
                parts   = ticker.rsplit("-", 1)
                eticker = parts[0] if len(parts) == 2 else ticker
                ...
                if eticker not in seen_event_tickers:
                    ...
                    for ev in all_events:
                        if ev["event_ticker"] == eticker:
                            # avoid duplicates
                            existing = {x.get("ticker") for x in ev["markets"]}
                            if ticker not in existing:
                                ev["markets"].append(m)
                            break
    except Exception as e:
        log.warning(f"[Fetcher] {series_ticker} active pass: {e}")
...
def _parse_market(raw: dict, label: str, series: str) -> Market:
    def f(key):
        ...
        try:
            ...
            return 0.0
    # /markets endpoint uses yes_bid/no_bid (cents int) OR yes_bid_dollars
    # normalise both formats to dollars
    def fd(dollars_key, cents_key=None):
    ...
    return Market(
        ...
        title      = raw.get("title", ""),
        yes_bid    = fd("yes_bid_dollars", "yes_bid"),
        yes_ask    = fd("yes_ask_dollars", "yes_ask"),
        no_bid     = fd("no_bid_dollars",  "no_bid"),
        no_ask     = fd("no_ask_dollars",  "no_ask"),
        last_price = fd("last_price_dollars", "last_price"),
        volume     = f("volume_fp"),
        liquidity  = fd("liquidity_dollars"),
        close_time = raw.get("close_time"),
        series     = series,
        label      = label,
        market_status = raw.get("market_status", "open"),
    )
...
def analyze_snapshot(snapshot: dict) -> list:
    from strategies import _allowed
    watchlist = []
    for sport, games in snapshot.items():
        for event_ticker, game in games.items():
            for label, markets in game.markets.items():
                for m in markets:
                    # Hard gate — only markets in ALLOWED_SERIES can ever
                    # become signals or watcher targets. Belt + suspenders.
...
def execute_signal(
    ...
    _tt = new_timer("SELL" if signal.action == "sell" else "BUY", signal.market_ticker)
    is_exit = signal.action == "sell"

    if not is_exit:
        # Second entry on same event requires higher confidence
        if getattr(signal, "second_entry", False) and signal.confidence < 0.68:
            log.info(f"[Executor] SKIP {signal.market_ticker} - 2nd entry conf {signal.confidence:.2f} < 0.68")
...
def run_bot():
    ...
    while True:
        ...
        try:
            ...
            # Give watcher the full ticker list for fast entry scanning
            all_tickers = [
                m.ticker
                for games in snapshot.values()
                for game in games.values()
                for mkts in game.markets.values()
                for m in mkts
            ]
            ...
            tg_ctrl.send_cycle_report(snapshot, signals, trades_placed, open_positions, total_pnl, daily_limit_hit)
            get_report().log_summary(threshold_ms=50.0)
            get_report().reset()
            get_report().start_cycle()
